		...
		if (e == 0) {
			...
			if (err) {
				...
				if (msg) {
					...
					[self setLastError:[NSString stringWithFormat:@"Error %ld occurred, no message",(long)e]];
				}
				CFRelease(err);
			} else {
				[self setLastError:[NSString stringWithFormat:@"Error %ld occurred, no error info",(long)e]];
			}
		}
	}
	CFRelease(op);
